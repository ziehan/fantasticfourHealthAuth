...
const LoginPage = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            if (!session) {
                setError("Sesi pengguna tidak ditemukan setelah login.");
                return;
            }

            const { data: profileData, error: profileError } = await supabase
                .from('profiles')
                .select('profession') // Hanya ambil kolom profession
                .eq('user_id', session.user.id) // Filter berdasarkan ID pengguna Supabase di kolom user_id
                .single(); // Ambil hanya satu baris data

            if (profileError) {
                setError(profileError.message || "Gagal mengambil data profil.");
                return;
            }

            // 4. Arahkan pengguna berdasarkan profession
            if (profileData && profileData.profession) {
                ...
                // Jika profession tidak ditemukan atau null, arahkan ke default
                router.push('/dashboard-nakes');
            }
        }
    };
