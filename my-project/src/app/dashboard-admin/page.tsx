...
import Navbar from './sections/navbar'; // Pastikan path ini benar
import Footer from './sections/footer'; // Pastikan path ini benar
...
import {
  ...
  WrenchScrewdriverIcon, ChartPieIcon, BanknotesIcon, CircleStackIcon // Icon untuk Data Master
...
interface Pasien {
  ...
  telepon: string; email?: string; alamat: string; tanggalRegistrasi: string; status: "Aktif" | "Nonaktif";
}
interface JanjiTemu {
...
// --- Data Dummy Awal (Sama seperti sebelumnya) ---
...
// --- Komponen Styling Umum (Sama seperti sebelumnya) ---
...
const whiteBoxClass = "bg-white/95 backdrop-blur-xl shadow-2xl rounded-2xl p-6 md:p-8 border border-white/20";
...
// --- Komponen AdminDashboardPageContent ---
const AdminDashboardPageContent: React.FC = () => {
  ...
  const handleSimpanDokter = (e: FormEvent) => { 
    e.preventDefault();
    ...
    if (editingDokter) {
      ...
      setDummyDokterList(prev => [{ id: newId, avatar: currentDokterFormData.avatar || `https://i.pravatar.cc/150?u=${newId}`, ...currentDokterFormData } as Dokter, ...prev]);
  ...
  const handleSimpanPasien = (e: FormEvent) => {
    ...
    if (editingPasien) {
      ...
      setDummyPasienList(prev => [{ id: newId, avatar: currentPasienFormData.avatar || `https://i.pravatar.cc/150?u=${newId}`, ...currentPasienFormData } as Pasien, ...prev]);
  ...
  const handleJanjiFormChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === "tanggalWaktu") setCurrentJanjiFormData(prev => ({...prev, [name]: new Date(value)}));
    else setCurrentJanjiFormData(prev => ({...prev, [name]: value}));
  };
  const handleSimpanJanji = (e: FormEvent) => {
    ...
    const janjiToAdd: JanjiTemu = {
        ...
        tanggalWaktu: new Date(tanggalWaktu), keluhan: keluhan as string, status: status as JanjiTemu['status']
    };
  ...
  const handleUbahStatusJanji = (janji: JanjiTemu, newStatus: JanjiTemu['status']) => { setDummyJanjiTemuList(prev => prev.map(jt => jt.id === janji.id ? {...jt, status: newStatus} : jt)); alert(`Status janji temu ${janji.id} diubah menjadi ${newStatus}.`); }

  return (
    <div className="flex flex-col min-h-screen"> {/* Wrapper utama */}
      <Navbar />
      <main className={`py-20 flex-1 ${darkBluePageGradient} relative selection:bg-[#A0D0D5] selection:text-[#1A0A3B] overflow-y-auto custom-scrollbar-dark`}>
        <FloatingGeometricShapes shapeClassName="bg-[#A0D0D5]/5" count={25} />
        
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
          <FadeInUp>
              <div className="flex flex-col sm:flex-row justify-between items-center mb-10">
                <h1 className={`text-3xl sm:text-4xl md:text-5xl font-bold text-white tracking-tight text-center sm:text-left`}>Dashboard Administrasi Sistem</h1>
                <button
                    onClick={() => {
                ...
                >
                    <CircleStackIcon className="w-5 h-5 mr-2.5" />
          ...
          <div className="space-y-10 md:space-y-12">
              ...
              <section id="ringkasan">
                  <FadeInUp className={whiteBoxClass}>
                      <h2 className={`text-xl md:text-2xl font-semibold mb-6 ${primaryDarkTextColor}`}>Ringkasan Umum</h2>
              ...
              <section id="laporan-analitik">
                   <FadeInUp delay="delay-[400ms]" className={whiteBoxClass}>
                      <h2 className={`text-xl md:text-2xl font-semibold mb-8 ${primaryDarkTextColor}`}>Pusat Laporan & Analitik</h2>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {[ {title: "Aktivitas Dokter", desc:"Jumlah pasien, jam praktik, diagnosis terbanyak.", icon: BriefcaseIcon, action:"Generate Laporan Dokter"}, {title: "Demografi Pasien", desc:"Distribusi usia, jenis kelamin, wilayah.", icon: UsersIcon, action:"Generate Laporan Pasien"}, {title: "Utilisaasi Janji Temu", desc:"Tingkat kehadiran, pembatalan, waktu tunggu.", icon: CalendarDaysIcon, action:"Generate Laporan Janji Temu"}, {title: "Penggunaan Obat", desc:"Tren peresepan, kategori obat populer, stok.", icon: QueueListIcon, action:"Generate Laporan Obat"}, {title: "Kinerja Keuangan", desc:"Ringkasan pendapatan, biaya operasional.", icon: BanknotesIcon, action:"Generate Laporan Keuangan"}, {title: "Log Aktivitas Sistem", desc:"Catatan perubahan data penting oleh admin.", icon: WrenchScrewdriverIcon, action:"Lihat Log Aktivitas"},
                          ].map(report => ( <div key={report.title} className={`bg-gray-50/90 border border-gray-200 rounded-xl p-6 shadow-lg hover:shadow-xl transition-shadow flex flex-col`}> <report.icon className={`w-10 h-10 mb-3 ${secondaryDarkTextColor} opacity-80`}/> <h4 className={`text-lg font-semibold mb-1.5 ${primaryDarkTextColor}`}>{report.title}</h4> <p className={`text-xs mb-5 ${subtleDarkTextColor} flex-grow`}>{report.desc}</p> <button onClick={() => alert(`${report.action} belum diimplementasikan.`)} className={`${adminPrimaryButtonClass} w-full mt-auto`}> <ArrowDownTrayIcon className="w-4 h-4 mr-2"/>Generate / Lihat </button> </div> ))}
                      </div>
                   </FadeInUp>
              </section>
          </div>
      ...
      <Footer />
    </div> // Penutup div utama flex-col min-h-screen
  );
}
