...
import Navbar from './sections/navbar'; // Path ini mengasumsikan 'sections' ada di dalam 'app/dashboard-nakes/'
import Footer from './sections/footer';   // Path ini mengasumsikan 'sections' ada di dalam 'app/dashboard-nakes/'
import Link from 'next/link';
import { useRouter } from 'next/navigation'; // Untuk navigasi yang lebih baik
...
import {
  ...
  ArrowRightIcon, // Ditambahkan untuk link ke Asisten AI
} from '@heroicons/react/24/outline';

import Chatbox from '@/components/chatbox'; // Path alias @/
...
const DashboardNakesPage: React.FC = () => {
  ...
  const filteredSchedule = useMemo(() => {
    let dataToFilter = [...scheduleData]; // Salin array agar tidak memodifikasi state langsung
    ...
    // Untuk filter 'semua' di scheduleDisplayData, tampilkan yang belum selesai dulu, lalu yang penting
    return dataToFilter.filter(item => item.status !== 'selesai').sort((a,b) => {
  ...
  const scheduleDisplayData = useMemo(() => {
    if (scheduleFilter === 'semua') {
      // Tampilkan yang belum selesai dulu (penting di atas), baru yang sudah selesai
      const notDone = scheduleData.filter(item => item.status !== 'selesai').sort((a,b) => {
        if (a.priority === 'penting' && b.priority !== 'penting') return -1;
        if (a.priority !== 'penting' && b.priority === 'penting') return 1;
        return 0;
      });
      const done = scheduleData.filter(item => item.status === 'selesai');
      return [...notDone, ...done];
    }
    return filteredSchedule; // Ini akan menggunakan hasil dari filteredSchedule untuk 'penting' dan 'selesai'
  ...
  const handleAddAgenda = (e: FormEvent) => {
    ...
    setScheduleData(prevSchedule => [agendaToAdd, ...prevSchedule].sort((a, b) => new Date(`1970/01/01 ${a.time.split(' - ')[0]}`).getTime() - new Date(`1970/01/01 ${b.time.split(' - ')[0]}`).getTime()));
  ...
  const handleStartLearning = (moduleId: number) => {
    ...
    alert(`Memulai modul: "${moduleTitle || 'Tidak Ditemukan'}"`);
  };
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow">
        ...
        <section className="min-h-screen h-auto flex flex-col items-center justify-center bg-gradient-to-br from-[#1A0A3B] via-[#1E47A0] to-[#1A0A3B] text-[#E0F2F3] text-center p-6 relative overflow-hidden pt-15">
          <FloatingParticles />
            ...
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight">
                Halo {profileData.name}! {/* Menggunakan nama dari profileData */}
            </h1>
            </FadeInUp>
            ...
            <LightBulbIcon aria-hidden="true" className="w-6 h-6 inline-block mr-2 mb-1" />
            &quot;{currentQuote}&quot;
            </FadeInUp>
            ...
            {quickStatsData.map((stat, index) => (
                ...
                <div className="bg-white/10 backdrop-blur-md p-5 rounded-xl border border-white/20 shadow-lg text-center h-full flex flex-col justify-center items-center">
                    <stat.icon aria-hidden="true" className={`w-10 h-10 mx-auto mb-3 ${stat.color}`} />
            ...
            {/* Tombol Isi Log Harian Anda */}
            <FadeInUp delay="delay-[800ms]" duration="duration-1000" className="mt-8">
                <button
                    aria-label="Isi Log Harian Anda"
                    className="bg-[#A0D0D5] text-[#1A0A3B] font-bold py-3.5 px-12 rounded-lg shadow-xl hover:bg-[#E0F2F3] transform hover:scale-105 transition-all duration-300 ease-in-out text-lg focus:outline-none focus:ring-4 focus:ring-[#A0D0D5]/50"
                    onClick={() => router.push('/dashboard-nakes/kuesioner')} // Menggunakan router.push
                >
                    Isi Log Harian Anda
                </button>
            </FadeInUp>

            {/* Tombol Mulai Konsultasi dengan Asisten AI */}
            <FadeInUp delay="delay-[900ms]" duration="duration-1000" className="mt-6">
              <Link href="/dashboard-nakes/assistant" legacyBehavior>
                <a className="inline-flex items-center justify-center px-8 py-4 bg-sky-500 text-white text-lg font-semibold rounded-xl shadow-lg hover:bg-sky-600 transition-transform transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-slate-900">
                  ...
                  <ArrowRightIcon aria-hidden="true" className="ml-3 h-5 w-5" />
                </a>
              </Link>
            </FadeInUp>

            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 animate-bounce opacity-70">
              <ChevronDownIcon aria-hidden="true" className="w-10 h-10 text-[#A0D0D5]" />
            </div>
        </section>
        ...
        <section className="min-h-screen h-auto flex flex-col items-center justify-center bg-[#E0F2F3] p-6 sm:p-8" id='Jadwal'>
            <div className="w-full max-w-5xl my-auto">
              ...
              <div className="space-y-4 max-h-[calc(100vh-300px)] sm:max-h-[calc(100vh-280px)] overflow-y-auto pr-2 custom-scrollbar">
                  ...
                  <FadeInUp key={item.id} delay={`delay-[${index * 50 + 200}ms]`} duration="duration-500">
                      <div 
                        ...
                        onKeyPress={(e) => e.key === 'Enter' && handleToggleAgendaStatus(item.id)} // Aksesibilitas keyboard
                        tabIndex={0} // Membuat div fokusabel
                        role="button" // Memberi peran tombol
                        aria-label={`Agenda: ${item.title}, Waktu: ${item.time}, Status: ${item.status === 'selesai' ? 'Selesai' : item.priority === 'penting' ? 'Penting' : 'Normal'}`}
                      >
                        ...
                        <div className={`p-2.5 rounded-full ${item.status === 'selesai' ? 'bg-gray-400/50' : item.priority === 'penting' ? 'bg-[#1E47A0]/20' : 'bg-[#A0D0D5]/30'}`}>
                            <CalendarDaysIcon aria-hidden="true" className={`w-7 h-7 ${item.status === 'selesai' ? 'text-gray-600' : item.priority === 'penting' ? 'text-[#1E47A0]' : 'text-[#1A0A3B]/70'}`} />
                        </div>
                        ...
                        {item.status !== 'selesai' && (
                            <CheckCircleIcon aria-hidden="true" className="w-6 h-6 text-green-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ml-2" title="Tandai Selesai"/>
                        )}
                        {item.status === 'selesai' && (
                            <ClipboardDocumentCheckIcon aria-hidden="true" className="w-6 h-6 text-gray-600 ml-2" title="Sudah Selesai"/>
                        )}
                      </div>
                  </FadeInUp>
                  )) : (
              ...
              <FadeInUp delay="delay-[500ms]" className="text-center mt-8">
                  ...
                  >
                    <PlusCircleIcon aria-hidden="true" className="w-5 h-5 mr-2" />
                    Tambah Agenda Baru
                  </button>
              </FadeInUp>
            </div>
                  ...
                  <div className="p-6 sm:p-10">
                    ...
                    <div className="md:grid md:grid-cols-3 md:gap-x-8 items-start">
                        <div className="md:col-span-1 text-center md:text-left mb-8 md:mb-0 flex flex-col items-center md:items-start">
                          ...
                          />
                          <button 
                            aria-label="Perbarui Profil"
                            className="mt-6 text-sm flex items-center text-[#1E47A0] hover:text-[#1A0A3B] font-semibold transition group">
                              <PencilSquareIcon aria-hidden="true" className="w-5 h-5 mr-1.5 transition-transform duration-300 group-hover:rotate-[-10deg]" />
                              Perbarui Profil
                          </button>
                        ...
                        <div className="md:col-span-2 space-y-2 text-sm text-[#1A0A3B]/90">
                          ...
                          {[
                              ...
                              { label: "Alamat", value: profileData.address, fullWidth: true }, // Contoh jika ada item yang ingin full width
                              { label: "FKTP", value: `${profileData.fktpName}, ${profileData.fktpAddress}`, fullWidth: true },
                              { label: "Wilayah Kerja", value: profileData.workArea },
                              { label: "No. STR", value: profileData.strNo },
                              { label: "No. SIP", value: profileData.sipNo },
                          ].map(item => (
                              // Menghapus className={item.fullWidth ? "col-span-2" : ""} karena properti fullWidth tidak ada di data.
                              // Jika ingin beberapa item full-width, tambahkan properti `fullWidth: true` ke item data tersebut.
                              <p key={item.label}>
                          ...
                          <div className="pt-4 mt-4 border-t border-[#1E47A0]/20">
                              <h4 className="text-lg font-semibold text-[#1A0A3B] mb-2 flex items-center">
                                <AcademicCapIcon aria-hidden="true" className="w-5 h-5 mr-2 text-[#1E47A0]"/>Keahlian Utama
                              </h4>
                          ...
                          <div className="pt-3 mt-3 border-t border-[#1E47A0]/20">
                              <h4 className="text-lg font-semibold text-[#1A0A3B] mb-2 flex items-center">
                                <LanguageIcon aria-hidden="true" className="w-5 h-5 mr-2 text-[#1E47A0]"/>Bahasa
                              </h4>
                              <ul className="list-disc list-inside ml-1 space-y-1">
            ...
            <div className="w-full max-w-6xl z-10">
              ...
              <FadeInUp delay="delay-[250ms]" className="mb-8 flex flex-col items-center">
                  <div className="relative flex items-center shadow-xl rounded-full w-full max-w-xl mb-4">
                    <label htmlFor="search-module" className="sr-only">Cari modul</label>
                    <input 
                        id="search-module"
                        type="search" 
                  ...
                  <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                    {trainingModuleCategories.map(category => (
                        <button key={category} onClick={() => setModuleCategoryFilter(category)}
                          aria-pressed={moduleCategoryFilter === category}
                        ...
                        >
                          <TagIcon aria-hidden="true" className="w-3 h-3 sm:w-4 sm:h-4 mr-1 inline-block"/>{category === 'semua' ? 'Semua Kategori' : category}
                        </button>
                    ))}
                  </div>
              </FadeInUp>
                      ...
                      <div className="bg-clip-padding backdrop-filter backdrop-blur-lg bg-[#A0D0D5]/30 border border-[#A0D0D5]/40 rounded-2xl shadow-xl p-6 flex flex-col h-full hover:shadow-[#A0D0D5]/30 transition-all duration-300 transform hover:-translate-y-2 group">
                        <div className="flex-grow mb-4">
                            <div className="flex justify-between items-start mb-2">
                                <module.icon aria-hidden="true" className="w-10 h-10 text-[#E0F2F3]/80 group-hover:text-[#E0F2F3] transition-colors"/>
                        ...
                        <button 
                            ...
                            aria-label={`Mulai belajar modul ${module.title}`}
                        ...
                        >
                            <PlayCircleIcon aria-hidden="true" className="w-6 h-6 mr-2" />
      ...
      <button
        ...
        title="Buka Asisten AI" // title bisa berfungsi sebagai tooltip dasar
        aria-label="Buka Asisten AI" // Lebih baik untuk aksesibilitas
        className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 bg-gradient-to-r from-[#1E47A0] to-[#123A7A] text-white p-4 rounded-full shadow-xl hover:from-[#123A7A] hover:to-[#1E47A0] transform hover:scale-110 transition-all duration-300 ease-in-out z-40"
      >
        <ChatBubbleLeftEllipsisIcon aria-hidden="true" className="w-7 h-7" />
      </button>
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button 
                  ...
                  aria-label="Tutup modal tambah agenda"
              ...
              <form onSubmit={handleAddAgenda} className="space-y-4">
                <div>
                  <label htmlFor="newAgendaTime" className="block text-sm font-medium text-[#1E47A0] mb-1">Waktu (cth: 09:00 - 10:00)</label>
                  <input type="text" name="time" id="newAgendaTime" value={newAgenda.time} onChange={handleNewAgendaChange} required
                ...
                <div>
                  <label htmlFor="newAgendaTitle" className="block text-sm font-medium text-[#1E47A0] mb-1">Judul Agenda</label>
                  <input type="text" name="title" id="newAgendaTitle" value={newAgenda.title} onChange={handleNewAgendaChange} required
                         className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#1E47A0] focus:border-[#1E47A0] sm:text-sm text-[#1A0A3B]" />
                </div>
                <div>
                  <label htmlFor="newAgendaType" className="block text-sm font-medium text-[#1E47A0] mb-1">Jenis Agenda</label>
                  <select name="type" id="newAgendaType" value={newAgenda.type} onChange={handleNewAgendaChange}
                ...
                <div>
                  <label htmlFor="newAgendaPriority" className="block text-sm font-medium text-[#1E47A0] mb-1">Prioritas</label>
                  <select name="priority" id="newAgendaPriority" value={newAgenda.priority} onChange={handleNewAgendaChange}
                          className="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-[#1E47A0] focus:border-[#1E47A0] sm:text-sm text-[#1A0A3B]">
          ...
          <FadeInUp duration="duration-300" className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col" style={{maxHeight: 'calc(100vh - 4rem)', height: '700px'}}>
            <div className="flex justify-between items-center p-4 sm:p-5 border-b border-[#A0D0D5]/50">
              <h3 className="text-xl font-bold text-[#1A0A3B] flex items-center">
                <ChatBubbleLeftEllipsisIcon aria-hidden="true" className="w-7 h-7 mr-3 text-[#1E47A0]" />
                Asisten AI Nakes
              </h3>
              <button 
                onClick={handleCloseAssistantModal} 
                aria-label="Tutup modal Asisten AI"
                className="text-[#1A0A3B]/70 hover:text-[#1A0A3B] p-1 rounded-full hover:bg-[#E0F2F3]/50">
            ...
            <div className="p-4 sm:p-5 border-b border-[#A0D0D5]/30 bg-[#E0F2F3]/60">
              <div className="flex items-start space-x-3">
                <div className="flex-shrink-0 pt-1">
                  <InformationCircleIcon aria-hidden="true" className="h-7 w-7 text-[#1E47A0]" />
                </div>
                <div>
                  <h2 className="text-lg font-semibold text-[#1A0A3B]">Selamat Datang, {profileData.name}!</h2>
