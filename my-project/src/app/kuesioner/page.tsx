  ...
  return (
    <FadeInUp className="mb-8 last:mb-0">
      <label className="block text-lg font-semibold text-[#1A0A3B] mb-1">
        {Icon && <Icon className="inline-block w-6 h-6 mr-2 mb-0.5 text-[#1E47A0]" />} {/* Penyesuaian: mb-0.5 */}
        {label}
      </label>
      {description && <p className="text-sm text-[#1A0A3B]/80 mb-3">{description}</p>}
      <div className={`grid ${type === 'checkbox' || options.length > 4 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'} gap-x-6 gap-y-4`}> {/* Penyesuaian: gap-y-4 */}
        {options.map((option) => (
          <label key={option.value} htmlFor={`${id}-${option.value}`} className="flex items-center space-x-3 p-3.5 border border-[#A0D0D5]/70 rounded-lg hover:border-[#1E47A0] hover:bg-[#E0F2F3]/70 transition-all cursor-pointer shadow-sm hover:shadow-md"> {/* Penyesuaian: p-3.5, shadow */}
            <input
              id={`${id}-${option.value}`}
            ...
            />
            <span className={`w-5 h-5 border-2 ${type === 'checkbox' ? 'rounded-md' : 'rounded-full'} border-[#A0D0D5] flex items-center justify-center transition-colors duration-200 peer-checked:bg-[#1E47A0] peer-checked:border-[#1E47A0]`}> {/* Penyesuaian: rounded-md untuk checkbox */}
              {type === 'checkbox' && (currentValue as string[]).includes(option.value) && <CheckIcon className="w-3.5 h-3.5 text-white" />} {/* Penyesuaian: ukuran checkicon */}
...
const LogHarianNakesPage: React.FC = () => {
  ...
  const sectionTitleClass = "text-xl sm:text-2xl font-bold text-[#1A0A3B] mb-6 pb-3 border-b-2 border-[#A0D0D5]/50 flex items-center"; // Penyesuaian: Ukuran font responsif, pb-3
  ...
  return (
    <div className={`min-h-screen ${pageBackgroundGradient} flex flex-col`}>
      ...
      {/* Kontainer utama untuk konten halaman, dengan padding atas untuk memberi ruang di bawah Navbar */}
<main className="flex-grow flex flex-col items-center pt-24 sm:pt-28 pb-8 sm:pb-12 px-4 sm:px-6 lg:px-8"> 
        {/* py-8 atau lebih besar untuk jarak dari navbar dan sebelum footer jika footer tidak fixed */}
        <FadeInUp className="w-full max-w-4xl" delay="delay-[100ms]">
          <header className="text-center mb-10 md:mb-12"> {/* Penyesuaian: margin bottom */}
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 md:p-10"> {/* Penyesuaian: rounded-2xl, padding responsif */}
            {/* --- Bagian 1: Log Harian Aktivitas Kerja --- */}
            <section className="mb-10 md:mb-12"> {/* Penyesuaian: margin bottom */}
              <h2 className={sectionTitleClass}>
                <ClockIcon className="w-7 h-7 mr-3 text-[#1E47A0]"/>1. Log Harian Aktivitas Kerja
              </h2>
              <div className="bg-[#E0F2F3] border border-[#A0D0D5] text-[#1A0A3B] px-4 py-3.5 rounded-lg mb-8 text-sm flex items-center shadow-sm"> {/* Penyesuaian: py-3.5, mb-8, shadow */}
                <CalendarDaysIcon className="w-5 h-5 mr-2.5 text-[#1E47A0]"/> {/* Penyesuaian: mr-2.5 */}
                Tanggal Pengisian: <span className="font-semibold ml-1.5">{currentDate}</span> (Otomatis terisi) {/* Penyesuaian: ml-1.5 */}
              </div>
            ...
            <FadeInUp delay="delay-[300ms]" className="mt-10 pt-8 border-t border-[#A0D0D5]/30 text-center"> {/* Penyesuaian: pt-8 */}
              <button 
                type="submit" 
              ...
              > {/* Penyesuaian: py-3.5, px-14, shadow-xl */}
                Kirim Log Harian
              </button>
            </FadeInUp>
          </form>
      ...
      <Footer /> {/* Footer dirender di sini */}
    </div>
  );
};
