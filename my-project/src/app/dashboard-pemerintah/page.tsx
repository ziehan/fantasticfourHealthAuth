...
// Komponen FadeInUp (opsional, bisa diambil dari contoh sebelumnya jika ingin ada animasi)
const FadeInUp: React.FC<{ children: React.ReactNode, delay?: string, className?: string }> = ({ children, delay = '', className = '' }) => {
  const [isVisible, setIsVisible] = useState(false);
  useEffect(() => {
    const timer = setTimeout(() => setIsVisible(true), 50); // Sedikit delay untuk memicu transisi
...
const QuestionGroup: React.FC<QuestionProps> = ({ id, label, description, options, type, currentValue, onChange, icon: Icon }) => {
  return (
    <FadeInUp className="mb-8 last:mb-0">
      <label className="block text-lg font-semibold text-[#1A0A3B] mb-1"> {/* Dark Text */}
        {Icon && <Icon className="inline-block w-6 h-6 mr-2 mb-1 text-[#1E47A0]" />} {/* Medium Blue for icon */}
        {label}
      </label>
      {description && <p className="text-sm text-[#1A0A3B]/80 mb-3">{description}</p>} {/* Dark Text with opacity or Muted Text if preferred */}
      <div className={`grid ${type === 'checkbox' || options.length > 4 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'} gap-x-6 gap-y-3`}>
        {options.map((option) => (
          <label key={option.value} htmlFor={`${id}-${option.value}`} className="flex items-center space-x-3 p-3 border border-[#A0D0D5]/70 rounded-lg hover:border-[#1E47A0] hover:bg-[#E0F2F3]/70 transition-all cursor-pointer"> {/* Border: Teal/Blue, Hover Border: Medium Blue, Hover Bg: Light Teal */}
            <input
              id={`${id}-${option.value}`}
            ...
            />
            <span className={`w-5 h-5 border-2 ${type === 'checkbox' ? 'rounded' : 'rounded-full'} border-[#A0D0D5] flex items-center justify-center transition-colors duration-200 peer-checked:bg-[#1E47A0] peer-checked:border-[#1E47A0]`}> {/* Border: Teal/Blue, Checked Bg & Border: Medium Blue */}
              {type === 'checkbox' && (currentValue as string[]).includes(option.value) && <CheckIcon className="w-3 h-3 text-white" />}
              {type === 'radio' && currentValue === option.value && <span className="w-2.5 h-2.5 bg-white rounded-full"></span>}
            </span>
            <span className="text-sm text-[#1A0A3B] peer-checked:font-semibold">{option.label}</span> {/* Dark Text */}
          </label>
        ))}
      </div>
    </FadeInUp>
...
const LogHarianNakesPage: React.FC = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    alert("Log harian berhasil dikirim! (Data di console)");
    setFormData(initialFormData);
  };
  
  // Menggunakan warna dari palet
  const sectionTitleClass = "text-2xl font-bold text-[#1A0A3B] mb-6 pb-2 border-b-2 border-[#A0D0D5]/50 flex items-center"; // Dark Text, Border: Teal/Blue
  const pageBackgroundGradient = "bg-gradient-to-br from-[#1A0A3B] via-[#1E47A0] to-[#123A7A]"; // Hero Section Gradient

  return (
    <div className={`min-h-screen ${pageBackgroundGradient} py-8 px-4 sm:px-6 lg:px-8 flex flex-col items-center`}>
      <FadeInUp className="w-full max-w-4xl">
        <header className="text-center mb-10">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-[#E0F2F3] tracking-tight">Log Harian Nakes</h1> {/* Light Text */}
          <p className="mt-3 text-lg text-[#A0D0D5]">Update kondisi dan aktivitas kerjamu hari ini.</p> {/* Secondary Light Text */}
        </header>

        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-2xl p-6 md:p-8 lg:p-10"> {/* White background for card */}
          {/* --- Bagian 1: Log Harian Aktivitas Kerja --- */}
          <section className="mb-12">
            <h2 className={sectionTitleClass}>
              <ClockIcon className="w-7 h-7 mr-3 text-[#1E47A0]"/>1. Log Harian Aktivitas Kerja {/* Medium Blue Icon */}
            </h2>
            <div className="bg-[#E0F2F3] border border-[#A0D0D5] text-[#1A0A3B] px-4 py-3 rounded-lg mb-6 text-sm flex items-center"> {/* Bg: Light Teal, Border: Teal/Blue, Text: Dark Text */}
              <CalendarDaysIcon className="w-5 h-5 mr-2 text-[#1E47A0]"/> {/* Medium Blue Icon */}
              Tanggal Pengisian: <span className="font-semibold ml-1">{currentDate}</span> (Otomatis terisi)
            </div>
          ...
          <section>
            <h2 className={sectionTitleClass}>
              <SparklesIcon className="w-7 h-7 mr-3 text-[#1E47A0]"/>2. Self-Assessment Kondisi Diri {/* Medium Blue Icon */}
            </h2>
          ...
          <FadeInUp delay="delay-[300ms]" className="mt-10 pt-6 border-t border-[#A0D0D5]/30 text-center"> {/* Border: Teal/Blue with opacity */}
            <button 
              type="submit" 
              className="bg-gradient-to-r from-[#1E47A0] to-[#123A7A] text-[#E0F2F3] font-bold py-3 px-12 rounded-lg shadow-lg transform hover:scale-105 hover:bg-gradient-to-r hover:from-[#A0D0D5] hover:to-[#c3e4e7] hover:text-[#1A0A3B] transition-all duration-300 ease-in-out text-lg focus:outline-none focus:ring-4 focus:ring-[#A0D0D5]/50"
            > {/* Button Gradient, Text: Light Text. Hover Button Gradient, Hover Text: Dark Text */}
              Kirim Log Harian
            </button>
          </FadeInUp>
        </form>
      </FadeInUp>
      {/* Footer telah dihilangkan sesuai permintaan */}
    </div>
  );
};
